...
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION aws_indonesia_islamic_finance_digital_BEDROCK_EAI
  ALLOWED_NETWORK_RULES = (ISLAMIC_DIGITAL_BANKING.APP.BEDROCK_NETWORK_RULE)
  ALLOWED_AUTHENTICATION_SECRETS = (ISLAMIC_DIGITAL_BANKING.APP.AWS_BEDROCK_SECRET)
  ENABLED = TRUE
  COMMENT = 'Bedrock access for Digital Islamic Banking';
...
CREATE OR REPLACE FUNCTION APP.BEDROCK_GENERATE(prompt VARCHAR)
  ...
  EXTERNAL_ACCESS_INTEGRATIONS = (aws_indonesia_islamic_finance_digital_BEDROCK_EAI)
  SECRETS = ('aws_creds' = ISLAMIC_DIGITAL_BANKING.APP.AWS_BEDROCK_SECRET)
AS $$
...
CREATE OR REPLACE PIPE RAW.REALTIME_PIPE
  AUTO_INGEST = TRUE
  INTEGRATION = 'aws_indonesia_islamic_finance_digital_S3_INT'
